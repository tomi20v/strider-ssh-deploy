...
function prepare(paths, stripTarFolder) {
  return [
    devnull(`rm -rf ${paths.old}`),
    devnull(`mv ${paths.remote} ${paths.old}`),
    devnull(`mkdir ${paths.remote}`),
    devnull(bundler.untarCmd(paths.bundle, paths.remote, stripTarFolder)),
        ...
        if (scp) {
          conn.sftp(function (err, sftp) {
            ...
            fs.createReadStream(scp.localBundlePath).pipe(str)
              .pipe(writeStream)
              .on('close', function () {
                runScript(conn, prepare(paths, scp.stripTarFolder).concat(script), out, function (_exitCode) {
                  exitCode = _exitCode;
                });
              });
          });
